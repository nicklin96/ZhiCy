...
public class CompatibilityChecker {
	...
	public boolean isSparqlCompatible3 (Sparql spq) 
	{
		...
		boolean shouldContinue = true;
		// shouldContinue实际是在判断某一个带variable的triple时，更新了variable fragment，即更新了[某个已经过检测的var]的可能types，这样需要按照新的vf从头检验sparql  
		// 为啥需要重新检验啊？每次join不是让候选集合减少的吗，好像一轮就够了吧；现在test一轮不重新检验的效果
		while (shouldContinue) 
		{
			shouldContinue = false;
			it = spq.tripleList.iterator();
	...
	public int hs_check5_V1PV2(Triple t) 
	{
		...
		{
			return 0;	//实际应该返回1，期待该次迭代后续的triple为V1,V2提供cands，下一轮时该triple即可起到check作用
		}
		else if(V2 == null)
		{
			if(V1.mayLiteral)
		...
		else
		{
			if(V1.mayLiteral)
				return 5;
			if(V2.mayLiteral || RelationFragment.isLiteral(pid))	//这个其实不一定，如果V1不符合出边check，也应该返回5，偷懒
				return 0;
